        ...
        <div className='w-11/12 mx-auto mb-6'>
            ...
            <div className='w-11/12 mx-auto my-4 flex items-center justify-between px-1'>
                <div className='xl:w-1/3 lg:w-1/4'>
                    <select aria-current={language} className='focus:outline-none rounded-lg bg-green-700  p-1 text-white w-full ' placeholder='Choose a language' onChange={(e) => handleLanguageSelection(e.target.value)}>
            ...
            {language === 'bn' && <div className='shadow-md rounded-md px-2 p-2 bg-white ' ref={applicationRef}>
                ...
                <div className='my-2 h-full p-2 text-lg    w-full'>
                    ...
                    <p className='text-black text-left'>- এই অনুমোদন পত্রটি ইস্যু হওয়ার তারিখ থেকে ১০  দিনের জন্য বৈধ এবং পুনর্নবীকরণ না করা পর্যন্ত স্বয়ংক্রিয়ভাবে মেয়াদ শেষ হয়ে যায়৷ এটিকে আমাদের দ্বারা প্রত্যাহার করা যেতে পারে এটির মুদ্রা চলাকালীন সময়ে আপনাকে জানানোর দিনগুলির সংখ্যা পরিবর্তন করার আগে৷</p>
            ...
            {language === 'hi' && <div className='shadow-md rounded-md px-2 p-2 bg-white' ref={applicationRef}>
                <div className='w-full '>
                    <img src="/tvslogo_2.png" className="w-52 object-fit" alt="tvs logo" height={50} width={50} />
                </div>
                <div className='w-full  mx-auto p-1 flex items-center justify-end'>
                    <div className='flex flex-col items-center'>
                        ...
                        <p className='text-lg text-center font-bold'>Email: helpdesk@tvscredit.com</p>
                        <div className='mx-auto text-left w-2/3'>
                            <p className='text-black text-lg font-bold'>दिनांक:
                                <span className='text-black text-left font-normal outline-none cursor-pointer hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition' onClick={() => handleSelection(variables[index].id)}>
                                    {variables[index].data === "" ? <b>{`[${variables[index].name}]`}</b> : <b>{`${variables[index].data}`}</b>}</span>
                            </p>
                        </div>
                ...
                <div className='mt-4 mb-8'>
                    <h2 className='text-center text-black text-2xl font-semibold underline font-serif'>स्वीकृति पत्र</h2>
                </div>

                <div className='my-2 h-full p-2 text-lg    w-full'>
                    <p className='text-black text-left font-semibold hover:font-bold hover:shadow-md w-fit   hover:bg-slate-300 duration-300 transition cursor-pointer' onClick={() => handleSelection(variables[index + 1].id)} >{variables[index + 1].data.length === 0 ? <b>{`[${variables[index + 1].name}]`}</b> : <b>{`${variables[index + 1].data}`}</b>}</p>
                    <p className='text-black text-left'>महोदय / महोदया,</p>
                    <p className='text-black text-left'>विषय : स्वीकृति पत्र - <span className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer' onClick={() => handleSelection(variables[index + 2].id)}>{variables[index + 2].data.length === 0 ? <b>{`[${variables[index + 2].name}]`}</b> : <b>{`${variables[index + 2].data}`}</b>}</span> डीलर प्रतिलिपि </p>
                    <br />
                    <p className='text-black text-left'>हमने अपने ग्राहक <span onClick={() => handleSelection(variables[index + 4].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>{variables[index + 4].data.length === 0 ? <b>{`[${variables[index + 4].name}]`}</b> : <b>{`${variables[index + 4].data}`}</b>}</span>
                        को <span onClick={() => handleSelection(variables[index + 3].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>{variables[index + 3].data.length === 0 ? <b>{`[${variables[index + 3].name}]`}</b> : <b>{`${variables[index + 3].data}`}</b>}</span>.
                        की खरीद के लिए एक ऋण सुविधा मंजूर की है। ऋण सुविधा के संबंध में ग्राहक द्वारा हमारे पक्ष में निष्पादित समझौते के अनुसार और ग्राहक के निर्देशों के आधार पर हम आपके पक्ष में निम्नलिखित राशि का वितरण कर रहे हैं।</p>
                    <br />
                    <p className='text-black text-left'>कृपया नीचे दिए गए पते पर ग्राहक को उत्पाद की डिलीवरी की व्यवस्था करें।</p>
                    <p className='text-black text-left'>ग्राहक का नाम: <span onClick={() => handleSelection(variables[index + 4].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>
                        <b>{variables[index + 4].data.length === 0 ? <b>{`[${variables[index + 4].name}]`}</b> : <b>{`${variables[index + 4].data}`}</b>}</b></span></p>

                    <p className='text-black text-left'>मोबाइल नंबर: <span onClick={() => handleSelection(variables[index + 5].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>
                        <b>{variables[index + 5].data.length === 0 ? <b>{`[${variables[index + 5].name}]`}</b> : <b>{`${variables[index + 5].data}`}</b>}</b></span></p>


                    <p className='text-black text-left'>ग्राहक का पता:<span onClick={() => handleSelection(variables[index + 6].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>
                        <b>{variables[index + 6].data.length === 0 ? <b>{`[${variables[index + 6].name}]`}</b> : <b>{`${variables[index + 6].data}`}</b>}</b></span></p>

                    <p className='text-black text-left'>उत्पाद ब्रांड: <span onClick={() => handleSelection(variables[index + 7].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'> <b>{variables[index + 7].data.length === 0 ? <b>{`[${variables[index + 7].name}]`}</b> : <b>{`${variables[index + 7].data}`}</b>}</b></span></p>
                    <p className='text-black text-left'>उत्पाद श्रेणी और मेक: <span onClick={() => handleSelection(variables[index + 8].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'><b>{variables[index + 8].data.length === 0 ? <b>{`[${variables[index + 8].name}]`}</b> : <b>{`${variables[index + 8].data}`}</b>}</b></span></p>
                    <p className='text-black text-left'>उत्पाद मॉडल: <span onClick={() => handleSelection(variables[index + 9].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'> <b>{variables[index + 9].data.length === 0 ? <b>{`[${variables[index + 9].name}]`}</b> : <b>{`${variables[index + 9].data}`}</b>}</b></span></p>
                    <p className='text-black text-left'>योजना कोड:<span onClick={() => handleSelection(variables[index + 10].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>
                        <b>{variables[index + 10].data.length === 0 ? <b>{`[${variables[index + 10].name}]`}</b> : <b>{`${variables[index + 10].data}`}</b>}</b></span>

                        और ईएमआई:<span onClick={() => handleSelection(variables[index + 11].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'><b>{variables[index + 11].data.length === 0 ? <b>{`[${variables[index + 11].name}]`}</b> : <b>{`${variables[index + 11].data}`}</b>}</b></span> </p>
                    <br />
                    ...
                    <table className='text-black text-center'>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><b>V. ग्राहक से एकत्र की जाने वाली कुल राशि</b></td>
                                <td onClick={() => handleSelection(variables[index + 20].id)} className='hover:font-bold hover:shadow-md p-1  hover:bg-slate-300 duration-300 transition cursor-pointer'>
                                    {variables[index + 20].data.length === 0 ? <b>{`[${variables[index + 20].name}]`}</b> : <b>{`${variables[index + 20].data}`}</b>}</td>
                    ...
                    <p className='text-black text-left'>- टीवीएस क्रेडिट सर्विसेज लिमिटेड को चिह्नित एचवाईपी के साथ चालान की मूल फाइनेंसर प्रति, विधिवत प्राप्त डिलीवरी की प्रति। चालान/चालान/डिलीवरी रसीद/मार्जिन मनी रसीद, हस्ताक्षर प्रमाण के अनुसार हस्ताक्षर के साथ एसीएच और सभी प्रासंगिक पूर्व संवितरण
                        अंतिम संवितरण राशि को संसाधित करने के लिए दस्तावेजों की स्पष्ट प्रति टीवीएस क्रेडिट सर्विसेज को भेजी जानी चाहिए। इसके अलावा, साथी ऐप होना चाहिए
                        लागू के रूप में स्थापित</p>
                    <p className='text-black text-left'>- वित्त पोषित उत्पाद केवल उपरोक्त ग्राहक पते पर वितरित किया जाना चाहिए। डिलीवरी के स्थान में परिवर्तन पर विचार नहीं किया जाएगा
                        किसी भी समय।</p>
                    <p className='text-black text-left'>- यह स्वीकृति पत्र जारी होने की तारीख से १० दिनों के लिए वैध है और जब तक नवीनीकरण नहीं किया जाता है तब तक स्वतः समाप्त हो जाता है। इसे रद्द भी किया जा सकता है
                        हमें इसके दौरान मुद्रा में डिलीवरी से पहले आपको सूचित करते हुए दिनों की संख्या बदल दी जाती है।</p>
                    <p className='text-black text-left'>- संपत्तियां जहां सीरियल नंबर सत्यापन अनिवार्य है, टीवीएस क्रेडिट सर्विसेज लिमिटेड की देयता केवल सफल सत्यापन के बाद उत्पन्न होती है
                        क्रमांक।</p>
                    <br />
                    <p className='text-black text-left font-bold underline'>भुगतान शर्तें</p>
                    <p className='text-black text-left'>- हम पारस्परिक रूप से सहमत शर्तों के अनुसार भुगतान जारी करेंगे।</p>
                    <p className='text-black text-left'><b>टिप्पणी: </b>यह कंप्यूटर जनित स्वीकृति पत्र हस्ताक्षर की आवश्यकता नहीं है।</p>
                    <p className='text-black text-left hover:font-bold hover:shadow-md w-fit  hover:bg-slate-300 duration-300 transition cursor-pointer' onClick={() => handleSelection(variables[index + 28].id)}  >
                        {variables[index + 28].data.length === 0 ? <b>{`[${variables[index + 28].name}]`}</b> : <b>{`${variables[index + 28].data}`}</b>}</p>
